...
def compile_latex(paper_dir: str, main_tex: str = "main.tex") -> bool:
    ...
    # Try latexmk first (handles bibtex + multiple passes automatically)
    cmd = ["latexmk", "-pdf", "-interaction=nonstopmode", "-file-line-error", main_tex]
    
    try:
        res = subprocess.run(cmd, cwd=paper_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, check=False)
        output = res.stdout
        
        errors = [line for line in output.splitlines() if "error:" in line.lower() or "! " in line]
        warnings = [line for line in output.splitlines() if "warning:" in line.lower() or "overfull" in line.lower()]
        ...
        subprocess.run(["pdflatex", "-interaction=nonstopmode", main_tex], cwd=paper_dir, check=False)
        subprocess.run(["bibtex", base], cwd=paper_dir, check=False)
        subprocess.run(["pdflatex", "-interaction=nonstopmode", main_tex], cwd=paper_dir, check=False)
        res = subprocess.run(["pdflatex", "-interaction=nonstopmode", main_tex], cwd=paper_dir, check=False)
        return res.returncode == 0

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compile LaTeX paper into PDF")
    parser.add_argument("--dir", "-d", type=str, default="paper_templates/arxiv_template", help="Directory containing main.tex")
    parser.add_argument("--main", "-m", type=str, default="main.tex", help="Main tex file name")
